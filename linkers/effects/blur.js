...
export async function socialBlur(multi, swalColour, format, targetFiles) {
    return new Promise((resolve) => {
        Swal.fire({
            ...
            preConfirm: (ratio) => {
                ...
                if (ratio === 'square') {
                    var cropSetting =
                        "[pxratio_fix]crop='if(lt(in_h,in_w),in_h,in_w):in_h'[cropped]";
                }
                targetFiles.forEach(function (fileSelected) {
                    let fileSettings = effectSetUp(fileSelected);
                    var finalOutput =
                        fileSettings.outputFile + '-bgblurred.mp4';
                        ...
                        .on('progress', function (progress) {
                            if (progress.percent === undefined) {
                                document.getElementById(
                                    'progressText'
                                ).textContent = progress.timemark.match(
                                    'dd:dd:dd)'
                                );
                            } else {
                                document.getElementById(
