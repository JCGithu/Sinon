    ...
    Swal.fire({
        ...
        preConfirm: (gifQual) => {
            if (gifQual == 'basic') {
                ...
                let finalOutputName =
                    path.parse(convertInfo.file).name + '.gif';
                if (convertInfo.inputExt.indexOf('.gif') >= 0) {
                    finalOutput =
                        convertInfo.outputFile + '-SinonConverted.gif';
                    finalOutputName =
                        path.parse(convertInfo.file).name +
                        '-SinonConverted.gif';
                }
                ...
                ffmpeg(convertInfo.file)
                    ...
                    .on('progress', function (progress) {
                        ...
                        let percentage = parseFloat(
                            (Math.round(progress.percent) / 100).toFixed(2)
                        );
                ...
                Swal.mixin({
                    ...
                    .then((result) => {
                        if (result.value) {
                            ...
                            let reRez =
                                '[frames]scale=w=trunc(oh*a/2)*2:h=480[rescaled]';
                            let cRange = '--colors=128';
                            let gifCrop;

                            if (rez == 'High') {
                                reRez =
                                    '[frames]scale=w=trunc(oh*a/2)*2:h=1080[rescaled]';
                            } else if (rez == 'hMid') {
                                reRez =
                                    '[frames]scale=w=trunc(oh*a/2)*2:h=720[rescaled]';
                            } else if (rez == 'hlow') {
                                reRez =
                                    '[frames]scale=w=trunc(oh*a/2)*2:h=360[rescaled]';
                            } else if (rez == 'low') {
                                reRez =
                                    '[frames]scale=w=trunc(oh*a/2)*2:h=240[rescaled]';
                            }
                            ...
                            if (crop == 'None') {
                                ...
                                gifCrop =
                                    "[rescaled]crop='in_w:if(lt(in_w,in_h),in_w*(9/16),in_h)'[cropped]";
                            } else if (crop == 'Square') {
                                gifCrop =
                                    "[rescaled]crop='if(lt(in_h,in_w),in_h,in_w):if(lt(in_w,in_h),in_w,in_h)'[cropped]";
                            ...
                            let OptimalOutput =
                                convertInfo.outputFile + '_advanced.gif';
