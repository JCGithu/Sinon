...
def check_directories(downloadPath, user_to_check):
    try:
        if not os.path.isdir(downloadPath + "/stories/{}/".format(user_to_check)):
            os.makedirs(downloadPath + "/stories/{}/".format(user_to_check))
        return True
    except Exception:
        return False


def get_media_story(ffmpegPath, downloadPath, user_to_check, user_id, ig_client, taken_at=False, no_video_thumbs=True, hq_videos=True):
        ...
        if hq_videos:
            ...
            for index, video in enumerate(list_video_v):
                ...
                if taken_at:
                    ...
                    final_filename = filename.split('.')[0] + ".mp4"
                save_path_video = downloadPath + "/stories/{}/".format(user_to_check) + final_filename.replace(".mp4", ".video.mp4")
            ...
            for index, video in enumerate(list_video):
                ...
                if taken_at:
                    ...
                    final_filename = filename.split('.')[0] + ".mp4"
                save_path = downloadPath + "/stories/{}/".format(user_to_check) + final_filename
...
def start(downloadPath, user_to_check, instaUse, instaPass, ffmpegPath):
    ...
    def download_user(user, attempt=0):
        try:
            if not user.isdigit():
                print ('user is not digit')
                user_res = (ig_client.username_info(user))
                user_id = repr(user_res['user']['pk']).encode('latin-1').decode("utf-8", errors='ignore')
                print (user_id)
            else:
                user_id = user
                user_info = (ig_client.user_info(user_id)).decode("utf-8", errors='ignore')
                ...
                if not user_info.get("user", None):
                    ...
                    print ('user reget resulted in: ' + user )
            print("[I] Getting stories for: {:s}".format(user))
            print('-' * 70)
            if check_directories(downloadPath, user_to_check):
                ...
                print ('folderpath=' + downloadPath + "/stories/{}/".format(user_to_check))
                get_media_story(ffmpegPath, downloadPath, user, user_id, ig_client)
        except Exception as e:
            if not attempt == 2:
                attempt += 1
                print("[E] ({:d}) Download failed: {:s}.".format(attempt, str(e)))
                ...
                time.sleep(5)
                print('-' * 70)
                download_user(user, attempt)
            else: 
                print("[E] Retry failed three times, skipping user.")
                print('-' * 70)

    download_user(user_to_check)